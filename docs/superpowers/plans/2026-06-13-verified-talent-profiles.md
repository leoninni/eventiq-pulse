...
<div className="flex gap-1 flex-wrap mb-5">
  {open.skills.map((s) => (
    <span key={s} className="text-xs px-2 py-0.5 rounded border border-primary/30 text-primary">
      {s}
    </span>
  ))}
</div>
    ...
    <div className="mb-5">
      ...
      <div className="flex gap-1.5 flex-wrap">
        {open.communityRoles.map((role) => (
          <div
            key={role}
            className="flex items-center gap-1.5 text-xs px-2 py-0.5 rounded border border-primary/20 bg-secondary/40 text-muted-foreground"
          >
