...
export function Events() {
  ...
  const openCandidates = openEvent
    ? eventCandidateMap[openEvent.id]
            ...
            <tbody>
              {events.map((e) => {
                ...
                return (
                  <tr
                    key={e.id}
                    className={`border-t border-border hover:bg-secondary/30 transition-colors ${best ? "bg-cyan/[0.03]" : ""}`}
                  >
                    <td className="px-4 py-3 font-medium relative">
                      {best && (
                        <span className="absolute left-0 top-2 bottom-2 w-[2px] rounded bg-cyan" />
                      )}
                      {e.name}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right tabular-nums">
                      €{e.sponsorship.toLocaleString()}
                    </td>
                    <td
                      className={`px-4 py-3 text-right tabular-nums ${best ? "text-cyan font-medium" : ""}`}
                    >
                      €{e.costPerLead}
                    </td>
        ...
        {openEvent && (
          <div className="p-6">
            ...
            <div className="text-xs text-muted-foreground">
              {openEvent.date} · {openEvent.location}
            </div>
            ...
            <div className="mt-6">
              <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">
                Candidates from this event
              </div>
              <div className="space-y-1.5">
                {openCandidates.map((c) => (
                  <div
                    key={c.id}
                    className="flex items-center gap-3 p-2.5 rounded-md border border-border bg-secondary/20"
                  >
