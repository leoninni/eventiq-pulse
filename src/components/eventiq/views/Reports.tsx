...
const generatedDays: Record<string, number> = {
  hacktum: 3,
  starthack: 18,
  codeberlin: 30,
  ethbuild: 60,
...
export function Reports() {
  ...
  if (ev) {
    const eventCandidates = eventCandidateMap[ev.id]
    ...
    const padded =
      ev.id === "hacktum"
        ? [
            { name: "Interested", v: 5 },
            { name: "In Review", v: 1 },
            { name: "Interviewed", v: 2 },
            { name: "Offer Extended", v: 1 },
            { name: "Rejected", v: 3 },
          ]
    ...
    return (
      <div className="p-4 md:p-8 max-w-[1100px]">
        <button
          ...
          className="text-sm text-muted-foreground hover:text-foreground inline-flex items-center gap-1.5 mb-5"
        >
        ...
        <div className="text-xs uppercase tracking-wider text-primary font-semibold mb-1">
          {ev.name.toUpperCase()} — Sponsor ROI Report
        </div>
        <div className="text-sm text-muted-foreground mb-6">
          Syntara Engineering · Generated May 2025
        </div>

        <div className="border-l-2 border-primary bg-primary/5 rounded-r-md p-4 mb-8">
          <p className="text-sm">
            <span className="font-semibold">{ev.name}</span> delivered{" "}
            <span className="font-semibold text-foreground">{ev.optIns} opt-in candidates</span> for
            Syntara, of which{" "}
            <span className="font-semibold text-foreground">
              {ev.pipeline} entered active pipeline
            </span>
            . At €{ev.sponsorship.toLocaleString()} sponsorship, your cost per qualified pipeline
            entry was <span className="font-semibold text-cyan">€{ev.costPerLead}</span> —{" "}
            <span className="text-success font-semibold">24% below</span> your 5-event average.
          </p>
        </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={padded} margin={{ left: 0, right: 12, top: 8, bottom: 8 }}>
                ...
                <Bar dataKey="v" radius={[6, 6, 0, 0]}>
                  {padded.map((d) => (
                    <Cell key={d.name} fill={statusColors[d.name as Status].dot} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <table className="w-full text-sm">
            <tbody className="divide-y divide-border">
              {[
                ...
                [
                  "Cost per hire",
                  ev.hires > 0 ? `€${Math.round(ev.sponsorship / ev.hires).toLocaleString()}` : "—",
                ],
              ...
              <tr>
                <td className="py-2 text-muted-foreground">Performance vs benchmark</td>
                <td className="py-2 text-right text-success font-semibold">24% better ✓</td>
              </tr>
            </tbody>
          </table>
        </Section>
        ...
        <Section title="4. Recruiter Notes">
          <textarea
            rows={4}
            placeholder="Add notes for internal use or to share with your team..."
        ...
        <div className="bg-card border border-border rounded-lg p-4 mt-6 text-sm">
          ...
          <span className="text-muted-foreground">
            Based on candidate quality and cost efficiency, we recommend sponsoring HackTUM again in
            2026. See AI Recommendations for the full Q3 event shortlist.
          </span>
        </div>
      </div>
    );
    ...
    <div className="p-4 md:p-8 max-w-[1100px]">
      <div className="mb-8">
        ...
        <p className="text-sm text-muted-foreground mt-2">
          Auto-generated ROI reports for each sponsored event.
        </p>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {withCost.map((e) => {
                const isBest =
                  e.costPerHire !== null && e.costPerHire === minCPH && minCPH !== maxCPH;
                const isWorst =
                  e.costPerHire !== null && e.costPerHire === maxCPH && minCPH !== maxCPH;
                const barWidth =
                  e.costPerHire !== null && e.costPerHire > 0
                    ? Math.round((minCPH / e.costPerHire) * 100)
                    : 0;
